...
async function renderAyuda() {
  ...
  main.innerHTML = `<h1>Trueque</h1>
    <div class="page">
      <p class="hint">El oficio se marca en el perfil. Acá solo se cruzan esas manos: a quién le falta lo tuyo, o a quién podés pedir.</p>
  ...
  async function loadTags() {
    ...
    paintOffered();
    paintNeeded();
    await Promise.all([loadSeekers(), loadMatches()]);
  }
  function paintOffered() {
    const box = document.getElementById("skill-offered");
    const mine = offeredSkills(tags);
    box.innerHTML = mine.length
      ? `${skillPillsHtml(tags)}<p class="hint"><a href="#/perfil">Cambiar oficios en el perfil</a></p>`
      : `<p class="empty">Todavía no contaste qué podés dar. <a href="#/perfil">Sumalo en tu perfil</a>.</p>`;
  }
  function paintNeeded() {
    const needed = document.getElementById("skill-needed");
    const shown = skillCatalog(tags);
    needed.innerHTML = shown.map((t) => `<button type="button" class="chip${t.requested ? " on" : ""}" data-slug="${escapeHtml(t.slug)}">${escapeHtml(t.label)}</button>`).join("");
  ...
  async function loadSeekers() {
    const box = document.getElementById("skill-seekers");
    if (!offeredSkills(tags).length) {
      box.innerHTML = "";
      return;
    }
    box.innerHTML = `<p class="empty">Buscando quién pide lo tuyo…</p>`;
    try {
      const rows = await api("/api/v1/timebank/seekers", {});
      const list = Array.isArray(rows) ? rows : [];
      box.innerHTML = list.length
        ? `<p class="hint section">Te buscan</p>${list.map((row) => skillPersonRow(row, row.tag, row.tagLabel)).join("")}`
  ...
  async function loadMatches() {
    ...
    if (!wanted.length) {
      box.innerHTML = `<p class="hint">Marcá qué necesitás para ver vecinos que lo dan.</p>`;
      return;
    }
    ...
    try {
      ...
      box.innerHTML = groups.length
        ? `<p class="hint section">Pueden ayudarte</p>${groups.map((row) => skillPersonRow(row, row.tagSlug, row.tagLabel)).join("")}`
    ...
    function paint() {
      ...
      if (mine) {
        box.innerHTML = `
          ...
          <div class="skill-add">
            <input id="skill-new" type="text" maxlength="40" placeholder="Ej. Costura, clases de guitarra" autocomplete="off">
...
const OGT_LOGO = "ogt-logo.png?v=crop";
...
function authStill(title, sub) {
  return `<section class="auth-still">
    <img src="${AUTH_STILL}" alt="Composición Quiet Studio: perro, manos, plantín y placa cívica">
